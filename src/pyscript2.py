...
def normalize_spectrogram(spectrogram_file):
    "Normalize spectrogram data in the .npy file"
    print(f"Normalizing spectrogram file: {spectrogram_file}")

    try:
        # Load the spectrogram from the .npy file
        spectrogram_db = np.load(spectrogram_file)

        # Apply Min-Max scaling to normalize the spectrogram to a range of [0, 1]
        min_val = np.min(spectrogram_db)
        max_val = np.max(spectrogram_db)
        normalized_spectrogram = (spectrogram_db - min_val) / (max_val - min_val)

        # Save the normalized spectrogram data as a .npy file
        output_normalized_file = spectrogram_file.replace('_spectrogram.npy', '_normalized.npy')
        np.save(output_normalized_file, normalized_spectrogram)
        print(f"Normalized spectrogram saved as '{output_normalized_file}'")

    except Exception as e:
        print(f"Error normalizing spectrogram {spectrogram_file}: {e}")
